...
{
    public class PersonagensController : Controller //A classe personagem herda (herança) da Controller 
    {
        public string uriBase = "http://KaPemi.somee.com/RpgApi/";
        ...
        public async Task<ActionResult> CreateAsync(PersonagemViewModel p)
        {
            ...
            HttpResponseMessage response = await httpClient.PostAsync(uriBase + "Personagens", content);
            string serialized = await response.Content.ReadAsStringAsync();
            await Task.Run(() => JsonConvert.DeserializeObject<List<PersonagemViewModel>>(serialized));
